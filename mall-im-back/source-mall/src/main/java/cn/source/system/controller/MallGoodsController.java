...
//    @PreAuthorize("@ss.hasPermi('system:goods:query')")
    ...
    @GetMapping("/pc-list")
    public TableDataInfo pcList(MallGoods mallGoods) {
        PageUtils.startPage();
        List<MallGoods> mallGoodsList = mallGoodsService.pcList(mallGoods);
        return getDataTable(mallGoodsList);
    }

    @PostMapping("/add-view")
    public AjaxResult addView(@RequestBody MallGoodReview mallGoodReview) {
        mallGoodsService.addView(mallGoodReview);
        return AjaxResult.success();
    }

    @GetMapping("/recommended")
    public AjaxResult recommended() {
        List<MallGoods> mallGoodsList = mallGoodsService.recommended();
        return AjaxResult.success(mallGoodsList);
    }
}
