      ...
      <div class="content-order">
        ...
        >
          ...
          <div class="item-desc">
            ...
            <div class="price">{{ item.price }} x {{ item.value }}</div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getOrder(id).then((response) => {
        ...
        this.mallOrderGoodsList = response.data.mallOrderGoodsList.map(item => ({
          ...item,
          image: 'http://127.0.0.1:8088' + item.image
        }))
        this.open = true
        this.title = '订单信息'
      })
